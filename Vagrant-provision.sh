...
APP_DB_PASS=defaultpasswd

# Edit the following to change the name of the database that is created (defaults to the user name)
APP_DB_NAME=projectflowdev
...
then
  ...
  echo "*:5432:$APP_DB_NAME:$APP_DB_USER:$APP_DB_PASS" >> ~/.pgpass
  chmod 0600 ~/.pgpass
...
-- Create the database:
CREATE DATABASE $APP_DB_NAME WITH OWNER $APP_DB_USER TEMPLATE template0 ENCODING 'UTF8' LC_COLLATE 'en_US.utf8' LC_CTYPE 'en_US.utf8';
EOF
